...
plane = p.loadURDF("C:/python/aeroXprojects/week2/task3/src/plane.urdf")     #Loading the plane
dronePos = [0,0,0.2]                     #Initial Position of the drone
drone = p.loadURDF("C:/python/aeroXprojects/week2/task3/src/quadrotor.urdf", dronePos)      #Loading the drone
...
while(True):
    ...
    if keycode.get(p.B3G_RETURN) == 1:        #If ENTER key is pressed then the simulation executes
        ...
        plane = p.loadURDF("C:/python/aeroXprojects/week2/task3/src/plane.urdf")        #Loading the plane and drone again
        dronePos = [0,0,0.1]
        drone = p.loadURDF("C:/python/aeroXprojects/week2/task3/src/quadrotor.urdf", dronePos)
        ...
        while(True):
            ...
            if keycode.get(p.B3G_RETURN) == 1:    #Reseting the simulation when Enter is pressed
                ...
                plane = p.loadURDF("C:/python/aeroXprojects/week2/task3/src/plane.urdf")
                dronePos = [0,0,0.2]
                drone = p.loadURDF("C:/python/aeroXprojects/week2/task3/src/quadrotor.urdf", dronePos)
                p.stepSimulation()
                break
